...
schemaCupom.path('dataInicial').validate(validarDataMaior, "Data informada `{VALUE}` para o campo `{PATH}` deve ser maior que a data corrente.");
...
schemaCupom.path('percentualDesconto').required(true, "Percentual de desconto é obrigatório");
schemaCupom.path('tipo').enum(enumTipo).trim();
...
const CupomDesconto = mongoose.model(
  "CupomDesconto",
  schemaCupom,
  "cupomdesconto"
);
